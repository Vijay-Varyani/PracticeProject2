...
node {
    ...
    withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
        stage('Deploye Code') {
			...
			if (isUnix()) {
				rmsg = sh returnStatus: true, script: "sfdx force:source:deploy --c --manifest manifest/package.xml -u ${HUB_ORG}"
			}else{
			   rmsg = bat returnStatus: true, script: "sfdx force:source:deploy --c --manifest manifest/package.xml -u ${HUB_ORG}"
			}
